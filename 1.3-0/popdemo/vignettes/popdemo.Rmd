...
The `project` function can now take a matrix, a list of matrices, or a special data structure called a `CompadreMSM` for the `A` argument. The `vector` can be a single vector, multiple column vectors in a matrix, `"n"` (a default option which projects a set of stage-biased vectors) or `"diri"` (which projects large set of random vectors). We will encounter all these options in this vignette. The `time` argument gives the number of projection intervals, but the output will have length `time` + 1, because the population size at time 0 is included.  

The `project` function returns an object of class 'Projection', containing the overall population size over time. The size of the object returned depends on the number of vectors requested. It is always a matrix with `time` + 1 rows. The number of columns returned is equal to the number of vectors specified by the call to `project`. In this case, it is only 1 (because we supplied a single numeric vector, `Tortvec1`).

The 'Projection' object also includes information on the time series of population vectors (i.e. (st)age-specific number / density). The first dimension is time and the second is (st)age, whilst the third represents each separate population projection. Therefore, for example, the first 10 timesteps of the third projection would be accessed using `object[1:11, ,3]`; the sizes of the second stage for all projections would be accessed using `object[ , 2, ]`; the size of the second stage at the 10th timestep of the second stage in the third projection would be `object[1:11, 2, 3]`.

We will access vectors for time intervals from 0 to 10:  
...
For a stochastic projection, the `project` function should usually have a list of matrices passed to the `A` argument, and a `vector`. The function will check that all the matrices have the same dimensions. `vector` can be either a single vector (as in the above case), or a matrix of several vectors, where each individual column represents one vector. The `time` argument gives the number of projection intervals, but the output will have length `time` + 1, because the population size at time 0 is included. There is one further important argument called `stochSeq`, which determines the sequence of matrices chosen for the projection. By default, `stochSeq = "unif"`. It's easy to see what the random sequence of matrices chosen is, how many matrices were used to make the projection, and what those matrices are:  
```{r}
stochSeq(Pbearp1.1)
...
We can pass this matrix to the `stochSeq` argument of the function to get a new estimate of population dynamics:
```{r}
Pbearp2.1 <- project(Pbear, Pbearvec, stochSeq = PbearM1, time = 50)
...
In this case, p = 0.5 means that there is now an equal probability of bad and good years. We can pass this to the `stochSeq` argument of the function to get a new estimate of population dynamics:
```{r}
Pbearp2.2 <- project(Pbear, Pbearvec, stochSeq = PbearM2, time = 50)
...
Pbearp2.3 <- project(Pbear, Pbearvec, stochSeq = PbearM3, time = 50)
...
```{r}
Pbearp2.4 <- project(Pbear, Pbearvec, stochSeq = PbearM4, time = 50)
...
Pbearp2.5 <- project(Pbear, Pbearvec, stochSeq = PbearM5, time = 50)
...
The final option for matrix selection is to give exactly the sequence of matrices to use. It's possible to force `project` to use a specific sequence of matrices, simply by passing a sequence of numbers or names to `stochSeq`. These numbers or names must correspond to the elements of your list of matrices.  
...
Pbearp3.1 <- project(Pbear, Pbearvec, stochSeq = Pbearseq)
plot(Pbearp3.1, log = "y")
```

You would get the same result using `stochSeq = rep(c("Y2001", "Y2002", "Y2003", "Y2004", "Y2005"), 10)`. The projection shows there's still a decline in overall population size, but with a clear cyclic dynamic to the trend.  
\  
...
```
[^11]: [L�fgren et al. (2000) Ann. Bot. Fennici, 37, 183-192.](http://www.sekj.org/PDF/anbf37/anbf37-183p.pdf)
...
*As we saw earlier, it's possible to force `project` to use a specific sequence of matrices, simply by passing a sequence of numbers or names to `stochSeq`. For this exercise in element selection, all we need to do is pass `1:30` to `stochSeq` and it selects the random matrices in order:*
```{r}
Thistlep1.1 <- project(ThistleS, vector = Thistlevec, stochSeq = 1:30)
...
```{r}
stoch(Pbear, c("lambda", "var"), vector = Pbearvec, stochSeq = PbearM1,
      time = 3000, discard = 100)

stoch(Pbear, c("lambda", "var"), vector = Pbearvec, stochSeq = PbearM2,
      time = 3000, discard = 100)

stoch(Pbear, c("lambda", "var"), vector = Pbearvec, stochSeq = PbearM3,
      time = 3000, discard = 100)

stoch(Pbear, c("lambda", "var"), vector = Pbearvec, stochSeq = PbearM4,
      time = 3000, discard = 100)

stoch(Pbear, c("lambda", "var"), vector = Pbearvec, stochSeq = PbearM5,
      time = 3000, discard = 100)
