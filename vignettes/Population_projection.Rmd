...
Population growth in an MPM is geometric: when you plot population size on a log scale (as we have here), it's easy to see that the population settles to a stable rate of decline. (As an aside, the long-term rate of growth/decline 'lambda' is equal to the dominant eigenvalue of the matrix and can be found using `eigs(Tort, "lambda")`). The dynamics that happen before this "stable state" is reached are called transient dynamics. 

Transient dynamics vary according to the population vector. 




There are some basic alterations you can make to projections: change the number of time intervals the projection runs for using the `time` argument. Use the `standard.A` argument to standardise the matrix to have 
