...
#'
#' @details 
#' \code{tfs_inertia} and \code{tfsm_inertia} differentiate a
#' transfer function to find sensitivity of inertia to perturbation.\cr
...
#' inertia.\cr\cr
#' \code{tfs_inertia} may be used to find sensitivity of a particular
#' perturbation structure. The perturbation structure is determined by 
#' \code{d\%*\%t(e)}. Therefore, the rows to be perturbed are determined by 
#' \code{d} and the columns to be perturbed are determined by \code{e}. 
#' The specific values in d and e will determine the relative perturbation 
#' magnitude. So for example, if only entry [3,2] of a 3 by 3 matrix is to be 
#' perturbed, then \code{d = c(0,0,1)} and \code{e = c(0,1,0)}. If entries 
#' [3,2] and [3,3] are to be perturbed with the magnitude of perturbation to 
#' [3,2] half that of [3,3] then \code{d = c(0,0,1)} and \code{e = c(0,0.5,1)}. 
#' \code{d} and \code{e} may also be expressed as numeric one-column matrices, 
#' e.g. \code{d = matrix(c(0,0,1), ncol=1)}, \code{e = matrix(c(0,0.5,1), ncol=1)}. 
#' Currently \code{tfa_inertia} can only work with rank-one, single-parameter 
#' perturbations: see Hodgson et al. (2006) for more information on perturbation 
#' structures.\cr\cr
#' \code{tfsm_inertia} returns a matrix of sensitivity values for
#' observed transitions, where the sensitivity of each matrix element is
#' evaluated seperately.\cr\cr
#' The formula used by \code{tfs_inertia} and \code{tfsm_inertia}
#' cannot be evaluated at lambda-max, therefore it is necessary to find the
#' limit of the formula as lambda approaches lambda-max. This is done using a
#' bisection method, starting at a value of lambda-max + \code{startval}.
#' \code{startval} should be small, to avoid the potential of false
#' convergence. The algorithm continues until successive sensitivity
#' calculations are within an accuracy of one another, determined by
#' \code{tolerance}: a \code{tolerance} of 1e-10 means that the sensitivity
#' calculation should be accurate to 10 decimal places. However, as the limit
#' approaches lambda-max, matrices become uninvertible (singular): if matrices
#' are found to be singular then \code{tolerance} should be relaxed and made
#' larger.\cr\cr
#' For \code{tfs_inertia}, there is an extra option to return and/or plot
#' the above fitting process using \code{return.fit=TRUE} and
#' \code{plot.fit=TRUE} respectively.\cr\cr
#'
#' @return 
...
#' \item{sens.fit}{the sensitivity values obtained in the fitting process.}
#' For \code{tfsm_inertia}, a matrix containing sensitivity of inertia
#' (or its bound) to each separate element of \code{A}.
#'
#' @references
#' \href{http://www.sciencedirect.com/science/article/pii/S0040580906000335}{
#' Hodgson \emph{et al}. (2006) Theor. Popul. Biol., 70, 214-224.}
#' \href{http://onlinelibrary.wiley.com/doi/10.1111/j.2041-210X.2012.00199.x/abstract{
#' Stott \emph{et al}. (2012) Methods Ecol. Evol., 3, 673-684.}\cr\cr
#'
#'
#' @family transfer function analyses
#' @family perturbation analyses
#'
#' @examples
#'   # Create a 3x3 matrix
...
#' 
#' @concept 
#' transfer function systems control resolvent demography transient dynamics
#' population momentum stable equivalent ratio amplification attenuation
#' population viability analysis PVA perturbation sensitivity
#'
#' @export
#'
tfs_inertia <-
