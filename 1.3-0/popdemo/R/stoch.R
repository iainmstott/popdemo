...
#'
#' @param object A list of matrices or a \code{CompadreMSM}. \code{stoch} uses
#'  \code{\link{project}} to 
#' perform a stochastic projection where the matrix varies with each timestep. 
#' The sequence of matrices is determined using \code{stochSeq}. Matrices must be 
...
#' @param stochSeq the sequence of matrices in a stochastic projection. 
...
#' @param time the number of projection intervals. The default is 1e+5.
...
#'   ( var_unif <- stoch(Pbear, what = "var", stochSeq = Pbear_seq) )
#'   
#' @concept 
...
setGeneric('stoch', function(object,...) {
  standardGeneric('stoch')
})
...
setMethod('stoch',
          ...
          function(object,
            ...
            A <- object@A
            if(length(A) < 2) {
              stop('object must contain multiple matrices')
            }
...
.stoch_impl <- function(A,
  ...
  # Moved some checks to methods, removed others because they will 
  # be done in the call to project (I think). This will need checking though...
  ...
  matDim <- dim(A[[1]])[[1]]
  if(is.null(vector)) vector <- stats::runif(matDim)
  ...
  gr <- pr[(discard:time) + 1] / pr[discard:time]
