...
#' \code{time+1}. If \code{vector="n"}, a numeric matrix of population 
#' projections: each column represents a single stage-biased projection and each 
#' column is of length \code{time+1}.\cr\cr
#' If \code{return.vec=TRUE}, a list with components:
#' \item{N}{the numeric vector or matrix of population sizes, as above.}
#' \item{vec}{
...
#' densities of one life stage in the projection. If \code{vector="n"}, a 
#' three-dimensional array of demographic vectors from projection of the set of 
#' stage-biased vectors through \code{A}. The first dimension represents time 
#' (and is therefore equal to \code{time+1} in length). The second dimension 
#' represents the densities of each stage (and is therefore equal to the 
#' dimension of \code{A} in length). The third dimension represents each 
#' individual stage-biased projection (and is therefore also equal to the 
#' dimension of \code{A} in length). For example, when projecting a 3 by 3 
#' matrix for >10 time intervals (see examples), the density of stage 3 in bias 
#' 2 at time 10 is found at element [11,3,2] (note that because element 1 
#' represents t=0, then t=10 is found at element 11); the time series of 
#' densities of stage 2 in bias 1 is found using [,2,1]; the matrix of 
#' population vectors for bias 2 would be found using [,,2].}    
#'
#' @seealso
#' S3 method for plotting population projections: \code{\link{plot.projection}}
#'
#' @examples
#'   # Create a 3x3 PPM
...
#'   
#' @concept 
#' projection project population size structure density 
#'
#' @export
#'
project <-
