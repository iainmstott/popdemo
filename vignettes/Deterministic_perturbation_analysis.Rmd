...
plot(delta, lambda_delta, type = "l", bty = "n")
``` 
\  
...
Where **d** represents the row to be perturbed, **e** represents the column to be perturbed, and $\delta$ is the perturbation magnitude. Using this notation, it can be shown[^1] that:

$$\delta^{-1} = \textbf{e}^\text{T}(\lambda\textbf{I} - \textbf{A})^{-1}\textbf{d}$$

[^1]: [Hodgson & Townley (2004) J. Appl. Ecol., 41, 1155-1161.](http://onlinelibrary.wiley.com/doi/10.1111/j.0021-8901.2004.00959.x/full)

Where $\lambda$ is the asymptotic growth of $textbf{A}_{\delta}$. Using these equations, it is possible to:
1. Decide on a maximum and minimum perturbation value
2. Compute $\lambda$ of $textbf{A}_{\delta}$ for max and min $\delta$
...
The `tfa_lambda` function calculates this transfer function ('tfa' stands for 'transfer function analysis'). For a perturbation to element [7,6]:
```{r} 
...
It's also possible to specify a target range of $\lambda$ instead using the `lambdarange` argument. This transfer function shows that the perturbation has a nonlinear effect on $\lambda$. The sensitivity is the slope where delta is equal to zero, which has an intercept at $\lambda_{max}$:
```{r echo=-1} 
plot(tf1)
s76 <- sens(Tort)[7, 6]
